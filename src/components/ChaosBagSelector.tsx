...
bagsMap.set('Curse of the Rougarou (Standard)', Bags.CurseOfTheRougarou.Standard);
bagsMap.set('Curse of the Rougarou (Hard)', Bags.CurseOfTheRougarou.Hard);
bagsMap.set('Carnevale of Horrors (Standard)', Bags.CarnevaleOfHorrors.Standard);
bagsMap.set('Carnevale of Horrors (Hard)', Bags.CarnevaleOfHorrors.Hard);
bagsMap.set('The Labyrinths of Lunacy (Standard)', Bags.TheLabyrinthsOfLunacy.Standard);
bagsMap.set('The Labyrinths of Lunacy (Hard)', Bags.TheLabyrinthsOfLunacy.Hard);
bagsMap.set('Guardians of the Abyss (Standard)', Bags.GuardiansOfTheAbyss.Standard);
...
export class ChaosBagSelector extends React.Component<ChaosBagSelectorProps> {
  ...
  public handleSelection(event: React.FormEvent<HTMLInputElement>) {
  ...
  public render() {
    ...
    return <Container className='p-3'>
      <select onChange={this.handleSelection.bind(this)}>
        {Array.from(bagsMap.keys()).map((k) => <option key={k} value={k}>{k}</option>)}
      </select>
    </Container>;
  }
